        ...
        private static readonly Dictionary<string, List<string>> _segmentMap = new Dictionary<string, List<string>>(StringComparer.OrdinalIgnoreCase)
    {
        ...
        private void loadSportTA(competitionEvent compEvent, string filename)
        {
            ...
            try
            {
                ...
                foreach (Competition comp in sportTAJOData.Event.Competitions)
                {
                    ...
                    {
                        //Skip if category name is "Tränare" or "Lagledare"
                        if (cat.Name == "Tränare" || cat.Name == "Lagledare")
                        {
                            continue;
                        }
        ...
        private static Dictionary<string, string> GetDataMap(categorySegment catSeg, participant p)
        {
            ...
            return new Dictionary<string, string>(StringComparer.OrdinalIgnoreCase)
            {
                ...
                { "{DisciplineNoSpace}", normalize(catSeg.discipline.Replace(" ","")) },
                { "{DisciplineDash}", normalize(catSeg.discipline.Replace(" ","-")) },
                { "{DisciplineUnderscore}", normalize(catSeg.discipline.Replace(" ","_")) },
                { "{Category}", normalize(catSeg.category) },
                { "{CategoryNoSpace}", normalize(catSeg.category.Replace(" ","")) },
                { "{CategoryDash}", normalize(catSeg.category.Replace(" ","-")) },
                { "{CategoryUnderscore}", normalize(catSeg.category.Replace(" ","_")) },
                { "{Segment}", normalize(catSeg.segment) },
                { "{SegmentNoSpace}", normalize(catSeg.segment.Replace(" ","")) },
                { "{SegmentDash}", normalize(catSeg.segment.Replace(" ","-")) },
                { "{SegmentUnderscore}", normalize(catSeg.segment.Replace(" ","_")) },
                ...
                { "{FirstNameUnderscore}", normalize(p.firstName?.Trim().Replace(" ","_")) },
